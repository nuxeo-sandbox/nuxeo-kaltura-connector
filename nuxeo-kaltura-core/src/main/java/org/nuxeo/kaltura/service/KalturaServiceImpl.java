...
import static org.nuxeo.kaltura.adapter.KalturaAdapter.KALTURA_FACET;
import static org.nuxeo.kaltura.adapter.KalturaAdapter.idXpath;

public class KalturaServiceImpl extends DefaultComponent implements KalturaService {

    private static final Log log = LogFactory.getLog(KalturaServiceImpl.class);
    ...
    @Override
    public void scheduleImport(DocumentModel root) {
    ...
    @Override
    public Object publish(DocumentModel doc) throws NuxeoException {
        KalturaClient client = getClient();
        Blob blob = (Blob) doc.getPropertyValue("file:content");
        String name = (String) doc.getPropertyValue("dc:title");
        try {
            String id =  client.getUploadService().upload(blob.getStream(),name,blob.getLength());
            KalturaMediaEntry entry = new KalturaMediaEntry();
            entry.name = name;
            entry.mediaType = KalturaMediaType.VIDEO;
            KalturaBaseEntry result =  client.getBaseEntryService().addFromUploadedFile(entry,id);
            doc.addFacet(KALTURA_FACET);
            doc.setPropertyValue(idXpath,result.id);
            return result;
        } catch (KalturaApiException | IOException e) {
            throw new NuxeoException(e);
        }
    }

    @Override
    public KalturaClient getClient() throws NuxeoException {
    ...
    protected DocumentModel getOrCreateFileDocument(DocumentModel root, String docType, String itemId){
        ...
        String query = String.format(
                "Select * From Document Where %s='%s' AND ecm:isCheckedInVersion = 0", idXpath,itemId);
